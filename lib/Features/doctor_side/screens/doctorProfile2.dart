...
class _DoctorPublicProfileScreenState
  ...
  Future<void> _checkIfAlreadyConnected() async {
    try {
      ...
      final connectedIds = data.map((d) {
        return (d['userId']?['_id'] ?? '').toString();
      }).toList();

      if (mounted) {
        setState(() {
          isAlreadyConnected = connectedIds.contains(widget.doctor.id);
          if (isAlreadyConnected) requestSent = true;
        });
      }
    } catch (_) {}
  }
  ...
  void initState() {
    super.initState();
    _checkIfAlreadyConnected(); // ← استدعي الفانكشن دي
  }
  @override
            ...
            child: Padding(
              ...
              child: Column(
                children: [
                  ...
                  // ── Request Button
               // ── Request Button — hide if already connected
if (!isAlreadyConnected)
  SizedBox(
    width: double.infinity,
    child: ElevatedButton.icon(
      style: ElevatedButton.styleFrom(
        backgroundColor: requestSent ? Colors.green : AppColors.blueColor,
        padding: const EdgeInsets.symmetric(vertical: 16),
        shape: RoundedRectangleBorder(
            borderRadius: BorderRadius.circular(16)),
      ),
      onPressed: requestSent || isRequesting ? null : sendRequest,
      icon: isRequesting
          ? const SizedBox(
              width: 20,
              height: 20,
